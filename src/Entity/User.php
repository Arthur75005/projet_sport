...
class User
{
    ...
    /**
     * @ORM\ManyToMany(targetEntity="App\Entity\Event", mappedyesBy="users")
     */
    private $groupEvents;
    ...
    public function __construct()
    {
        ...
        $this->groupTeams = new ArrayCollection();
     ...
     */
    public function getGroupeTeams(): Collection
    {
        return $this->groupeTeams;
    }

    public function addGroupeTeam(Team $groupeTeam): self
    {
        if (!$this->groupeTeams->contains($groupeTeam)) {
            $this->groupeTeams[] = $groupeTeam;
            $groupeTeam->setUser($this);
        }

        return $this;
    }

    public function removeGroupeTeam(Team $groupeTeam): self
    {
        if ($this->groupeTeams->contains($groupeTeam)) {
            $this->groupeTeams->removeElement($groupeTeam);
            // set the owning side to null (unless already changed)
            if ($groupeTeam->getUser() === $this) {
                $groupeTeam->setUser(null);
            }
        }
